...
README_START = "<!-- BEGIN GENERATED FINDINGS -->"
README_END = "<!-- END GENERATED FINDINGS -->"


def write_readme_findings(s6, s7, s8, ident):
    """Regenerate the README's findings block from the artefacts.

    The README is the first thing a reader sees, and it had drifted badly: it still
    advertised the v1 campaign's headline that the post-allocation acceptance check
    was "the largest effect measured" at 1.42-1.60 m, when a correctly isolated
    ablation puts it at exactly zero. Hand-maintained summaries drift, so this block
    is generated from the same JSON the report reads and is rewritten on every run.
    """
    if not (s6 and os.path.exists("README.md")):
        return
    pe, tab = s6.get("paired_effects", {}), s6["table"]
    conds = ("healthy", "actuator", "perception", "combined")

    def eff(pair, key="post_onset_rmse"):
    ...
    def line(pair, label, note=""):
        e = eff(pair)
        if not e:
            return None
        return (f"- **{label}:** {e['lo']:+.3f} to {e['hi']:+.3f} m post-onset RMSE; "
                f"significantly better in {e['better']}/{e['n']} conditions, "
                f"significantly worse in {e['worse']}/{e['n']}.{note}")

    n_ep = sum(v["n_episodes"] for v in tab.values())
    n_draw = s6.get("n_test_per_condition")
    L = [README_START, "",
         "## Findings", ""]
    if ident:
        L += [f"Run `{ident['run_id']}`, commit "
              f"`{str(ident.get('git_commit', ''))[:8]}`, config "
              f"`{ident.get('config_manifest_hash', '')}`. "
              f"{n_ep} rollouts over {n_draw} matched scenario draws per condition "
              f"x 3 training seeds.", "",
              "*This section is generated by `stage9_writeup.py`. Do not hand-edit: "
              "earlier hand-written", "summaries drifted from the artefacts and "
              "advertised conclusions the data had already", "overturned.*", ""]

    L += ["### Supported", ""]
    sup = [
        line("no_impact-constant_context",
             "Inferring a *changing* context helps (M2 vs M1)",
             " This is the clean isolation: neither side carries the behavioural "
             "loss."),
        line("full-nominal_recovery", "The learned residual helps (M3 vs M0)"),
    ]
    hp = (s8 or {}).get("horizon", {}).get("horizon_pair", {})
    if hp:
        big = max(abs(v["mean"]) for v in hp.values())
        nsig = sum(1 for v in hp.values() if not v["significant"])
        sup.append(
            f"- **No conclusion depends on the disputed horizon.** The manuscript "
            f"states N=10 and the build spec N=12; neither was verified against the "
            f"flight configuration, so both were run. Largest paired difference "
            f"{big:.4f} m, not significant in {nsig}/{len(hp)} conditions.")
    L += [s for s in sup if s] + [""]

    L += ["### Not supported", ""]
    ns = []
    e = eff("full-no_impact")
    if e:
        ns.append(
            f"- **The behavioural-supervision gain (M3 vs M2), the paper's primary "
            f"claim.** It is *adverse*: {e['lo']:+.3f} to {e['hi']:+.3f} m, "
            f"significantly worse in {e['worse']}/{e['n']} conditions. The "
            f"pre-declared lambda_I grid also selected **zero**, so the "
            f"positive-weight model is an ablation rather than the selected method.")
    e = eff("full-full_no_check")
    if e and abs(e["lo"]) < 1e-9 and abs(e["hi"]) < 1e-9:
        ns.append(
            "- **The post-allocation decrease test of Eq. (18) (M3 vs M4).** As a "
            "true single-component ablation its effect is **exactly 0.00000 m in all "
            "four conditions**: it never rejects a candidate. The eta-free "
            "first-action condition is the same inequality without the allowance, so "
            "it is strictly tighter and screens out everything the decrease test "
            "would catch. An earlier campaign reported this check as the largest "
            "effect measured; that came from three safeguards being disabled at once.")
    e = eff("full-fallback_only")
    if e:
        ns.append(
            f"- **MPC planning beyond the fallback (M3 vs M5).** "
            f"{e['lo']:+.3f} to {e['hi']:+.3f} m: deleting the optimiser and keeping "
            f"the checked fallback changes little.")
    if s7:
        sw = s7.get("sweep", [])
        ncert = sum(1 for r in sw if r.get("certified"))
        ns.append(
            f"- **The numerical recovery certificate.** {ncert} of {len(sw)} swept "
            f"cells certify, now with a *sound* enclosure. The residual admissibility "
            f"analysis did **not** run this campaign (`residual: {{}}`), so the empty "
            f"region cannot be attributed to residual Lipschitz growth.")
    ...
    ns.append(
        f"- **The declared task specification.** Reaching the *final* waypoint within "
        f"{spec.get('tol_pos_m', 0.15):.2f} m and "
        f"{spec.get('tol_yaw_deg', 5):.0f} deg and holding it "
        f"{spec.get('dwell_s', 2):.0f} s happens in only {tot_ts}/{tot_def} rollouts "
        f"across all methods. The evaluated controllers rarely achieve it; that is not "
        f"evidence it is unattainable in principle.")
    L += [s for s in ns if s] + [""]

    sup_share = [tab[f"{c}|full"]["frac_supervisor"]["mean"] for c in conds
                 if f"{c}|full" in tab]
    ...
    if sup_share:
        L += ["### What the evaluated policy actually is", "",
              f"Worth stating plainly, because it conditions every row above: under "
              f"M3 the fixed velocity-damping supervisor issues "
              f"{min(sup_share):.0%}-{max(sup_share):.0%} of transmitted actions, the "
              f"eta-free first-action condition diverts a further "
              f"{min(fa):.0%}-{max(fa):.0%}, and the MPC candidate is transmitted on "
              f"only {min(cand):.0%}-{max(cand):.0%} of samples. What the table scores "
              f"is largely a fixed feedback law, not context-conditioned MPC.", ""]

    L += ["### Known gaps, not yet implemented", "",
          "- One canonical `f_theta` evaluator shared by training, feedforward, "
          "horizon prediction, affine construction and checking; the feedforward is "
          "still nominal and its learned residual is computed and discarded.",
    ...
    if README_START in txt and README_END in txt:
        pre = txt.split(README_START)[0]
        post = txt.split(README_END)[1]
        txt = pre + block + post
    else:
        # first run: replace a hand-written Findings section if present
        import re
        m = re.search(r"\n## Findings\n.*?(?=\n## )", txt, flags=re.S)
        txt = (txt[:m.start()] + "\n" + block + "\n" + txt[m.end():]) if m \
            else txt.rstrip() + "\n\n" + block + "\n"
    with open("README.md", "w") as f:
        f.write(txt)
    print("regenerated README.md findings block")
...
def main():
    ...
    write_readme_findings(s6, s7, s8, ident)
