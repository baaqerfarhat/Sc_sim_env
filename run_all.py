...
Stages are gates as much as steps: 1-4 hold the simulation to the measured hardware
envelopes and nothing downstream is meaningful if they fail, so the run stops on the
first non-zero exit rather than carrying on and producing numbers nobody should quote.
...
Stage 5 (data generation and training) and Stage 6 (the 2400-episode grid) dominate the
...
STAGES = {
    ...
    3: ("stage3_estimator.py", "estimator surrogate fit + perception degradation",
        "3 min"),
    5: ("stage5_data_train.py", "episodes, probe branches, 9 trained models", "70 min"),
    6: ("stage6_methods.py", "calibration + the matched comparison grid", "15 min"),
...
ORDER = [1, 2, 3, 5, 6, 7, 8, 9, 91]

# stages 1-4 are gates: a failure there invalidates everything after it
GATES = {1, 2, 3}
